...
ascidat <- read.csv('https://raw.githubusercontent.com/fawda123/ASCIfigs/master/raw/asci.scores.csv', stringsAsFactors = F) %>% 
  ...
  mutate(
    Replicate = gsub('.*_([0-9]+)$', '\\1', X), 
    X = gsub('_[0-9]+$', '', X),
    SampleDate = gsub('.*_(.*)$', '\\1', X),
    StationID = gsub('(^.*)_.*$', '\\1', X)
  ) %>% 
  select(-X)
...
locs <- read.csv('https://raw.githubusercontent.com/fawda123/ASCIfigs/master/raw/csci.scores.csv', stringsAsFactors = F) %>% 
  mutate(
    Replicate = gsub('.*_([0-9]+)$', '\\1', SampleID_old), 
    SampleID_old = gsub('_[0-9]+$', '', SampleID_old),
    SampleDate = gsub('.*_(.*)$', '\\1', SampleID_old),
  ...
  select(StationID, New_Lat, New_Long) %>% 
  unique
